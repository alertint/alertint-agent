...
// probe drive unavailable, verification_rejudge and semantic_profile alone
// drive degraded, and this order coincides exactly with the priority the
// state itself implies. triage_draft and assessment are treated as peers
// (spec.md: "LLM health remains one installation-level capability state fed
// by real Acute Triage and Assessment outcomes") — both are primary,
// first-class model capabilities the product's core loop depends on (Acute
// Triage judges Incidents, the Situation controller's L2 dispatch judges
// Situations), so either one failing means the installation is genuinely
// unavailable, not merely degraded. semantic_profile is a real,
// regularly-succeeding shared-primary capability (unlike query_repair's
// opportunistic-only successes) but is advisory-only (Plan 4: it never
// gates or blocks Situation lifecycle), so it gets verification_rejudge's
// own treatment — reported and rolled up, but only into Degraded, never
// Unavailable. memory_classifier and query_repair are reported per
...
func (t *Tracker) aggregate() (State, string, string) {
	order := []Capability{
		CapabilityTriageDraft, CapabilityAssessment, CapabilityProbe,
		CapabilityVerificationRejudge, CapabilitySemanticProfile,
	}
