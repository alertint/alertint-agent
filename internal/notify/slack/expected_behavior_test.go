...
func TestExpectedBehaviorStoppedReasonsAndWithdrawalUseSimpleText(t *testing.T) {
	...
	if err != nil || msg.Text != "default removed the expected schedule. Normal assessment resumes." {
	...
	msg, err = RenderSituationJournal(tr)
	wantApplied := "This condition matches default's expected schedule until " + SlackDateToken(boundary, "{time}") + ". Monitoring continues."
