...
func TestUsageStats_AggregatesWithinWindow(t *testing.T) {
	...
	slackSent := func(event string) {
		t.Helper()
		if err := a.Append(ctx, "notify.slack", "notify.sent", map[string]any{
			"incident_id": "inc-1", "event": event, "recipient": "slack",
		}); err != nil {
	...
	slackSent("firing")                                                            // t=10:03 — a new card: one poke
	slackSent("firing")                                                            // t=10:04 — another card
	slackSent("rejudge")                                                           // t=10:05 — in-place edit, not a poke
	slackSent("resolved")                                                          // t=10:06 — resolved update, not a poke
	mustAppend(t, a, ctx, "notify.slack", "notify.skipped")                        // t=10:07
	mustAppend(t, a, ctx, "notify.stdout", "notify.sent")                          // t=10:08 — not Slack, must be excluded
	mustAppend(t, a, ctx, "skill:acute-triage", "incident.analyzed")               // t=10:09
	mustAppend(t, a, ctx, "skill:acute-triage", "incident.analyzed")               // t=10:10
	mustAppend(t, a, ctx, "skill:acute-triage", "incident.analysis_failed")        // t=10:11 — retryable attempt failure, not terminal
	mustAppend(t, a, ctx, "skill:acute-triage", "incident.analysis_failed")        // t=10:12 — same incident, retried again
	mustAppend(t, a, ctx, "correlator", "incident.triage_exhausted")               // t=10:13 — terminal: one failed incident
	mustAppend(t, a, ctx, "llm.anthropic", "llm.request")                          // t=10:14 — request, not response
	mustAppendPayload(t, a, ctx, "alertmanager", "alert.received", map[string]any{ // t=10:15 — one delivery, three alerts
		"alert_count": 3, "persisted_count": 3,
	})
	mustAppendPayload(t, a, ctx, "alertmanager", "alert.received", map[string]any{ // t=10:16 — repeat delivery of a known alert still counts
		"alert_count": 1, "persisted_count": 1,
	})
	mustAppendPayload(t, a, ctx, "zabbix", "alert.received", map[string]any{ // t=10:17 — Zabbix rows carry no alert_count: one alert
		"event_id": "42", "persisted": true,
	})
	llmResponse("llm.anthropic", "claude-x", 999, 999, 0, 0) // t=10:18 — outside window (until excludes it)

	since := base
	until := base.Add(18 * time.Minute) // half-open: excludes the t=10:18 row
	...
	if got.SlackCardsPosted != 2 || got.SlackSkipped != 1 {
		t.Errorf("Slack = cards:%d skipped:%d, want cards:2 skipped:1", got.SlackCardsPosted, got.SlackSkipped)
	}
