...
func TestApplyDelivery_TerminalSituationOwnerOpensFreshIncident(t *testing.T) {
	...
	claims, err := st.ClaimSituationInputs(context.Background(), "situation-worker", now.Add(time.Minute), time.Minute, 10)
	if err != nil {
		t.Fatal(err)
	}
	var applied bool
	for _, input := range claims {
		if input.IncidentID != owner {
			continue
		}
		if err := st.ApplySituationInput(context.Background(), input); err != nil {
			t.Fatal(err)
		}
		applied = true
	}
	if !applied {
		t.Fatal("fresh incident input was not claimable")
	}
	situations, err := st.ListSituations(context.Background(), 10)
	if err != nil {
		t.Fatal(err)
	}
	var linked bool
	for _, situation := range situations {
		if situation.PreviousSituationID != nil && *situation.PreviousSituationID == "sit_1" {
			linked = true
		}
	}
	if !linked {
		t.Fatal("fresh incident did not open a Situation linked to terminal predecessor sit_1")
	}
}
