...
func TestControllerDeliveryOverlayPreservesObservationDeadline(t *testing.T) {
	now := ctBaseTime.Add(48 * time.Hour)
	in := ctBaseSnapshotInput()
	d := &in.Deliveries[0]
	d.ReceivedAt = now.Add(-25 * time.Hour)
	d.AcquisitionMode = "webhook"
	in.Prepared = situation.PreparedState{CycleID: "expired-overlay", Lifecycle: []situation.SourceObservation{{
		AlertID: d.AlertID, EpisodeKey: d.EpisodeKey, State: situation.SourceStateFiring,
		ObservedAt: d.ReceivedAt, DeadlineAt: now.Add(-time.Hour), AcquisitionMode: "webhook",
	}}}
	got := reconcileDeliveryLifecycle(t, in, now)
	if got.Lifecycle != model.LifecycleClosedUnknown {
		t.Fatalf("expired delivery overlay lifecycle=%s, want closed_unknown", got.Lifecycle)
	}
	// The same deadline rule applies when stale prepared detail was withheld.
	in.Prepared.Lifecycle = nil
	got = reconcileDeliveryLifecycle(t, in, now)
	if got.Lifecycle != model.LifecycleClosedUnknown {
		t.Fatalf("delivery without prepared detail lifecycle=%s, want closed_unknown", got.Lifecycle)
	}
}
