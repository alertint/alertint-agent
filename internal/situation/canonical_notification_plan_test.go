...
func TestCanonicalTriageRecoveryOverlapCreatesOneLogicalRecoveryReply(t *testing.T) {
	in := canonicalPublication(time.Date(2026, 9, 11, 10, 0, 0, 0, time.UTC), model.LifecycleRecoveryPending, 0,
		model.CandidateUsefulFinding, model.CandidateAllClear)
	in.RootPublished = true
	in.Transitions[0].Projection.Briefing.Work.Phase = model.WorkPhaseExecuting

	var recovery int
	for _, kind := range replyKinds(hsPlan(t, in)) {
		if kind == model.ReplyRecoveryObserved {
			recovery++
		}
	}
	if recovery != 1 {
		t.Fatalf("recovery replies = %d, want one logical Situation recovery reply", recovery)
	}
}
