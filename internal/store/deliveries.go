...
// unresolvedIncidentMembersTx counts the Incident's current members whose
// authoritative immutable source episode is not resolved. A delivery has one
// correlation owner, but an Alert may legitimately belong to more than one
...
func correlatedSituationInputKindTx(ctx context.Context, tx *sql.Tx, requested, incidentID, deliveryStatus, incidentStatus string, created bool, now time.Time) (kind string, resolved bool, err error) {
	...
	if deliveryStatus == "firing" && incidentStatus == "resolved" {
		res, err := tx.ExecContext(ctx, `
			UPDATE incidents SET status = 'analyzed', updated_at = ?
			WHERE id = ? AND status = 'resolved'`, canonicalTime(now), incidentID)
		if err != nil {
			return "", false, fmt.Errorf("store: reopen correlated incident during nonterminal recurrence: %w", err)
		}
		if changed, err := res.RowsAffected(); err != nil {
			return "", false, fmt.Errorf("store: count reopened correlated incident: %w", err)
		} else if changed != 1 {
			return "", false, ErrIncidentOwnerNotCollapsible
		}
	...
	if changed == 1 {
		if _, err := tx.ExecContext(ctx, `DELETE FROM incident_triage WHERE incident_id=? AND phase!='exhausted'`, incidentID); err != nil {
			return "", false, fmt.Errorf("store: clear resolved incident triage: %w", err)
		}
...
// reconcileResolvedMembershipsTx carries one applied recovery to every other
// nonterminal Incident that already contains the Alert. The delivery keeps one
// immutable correlation owner; secondary memberships receive deterministic,
// delivery-less Situation inputs and consume the same applied Alert history
// when their Situation snapshot is loaded.
func reconcileResolvedMembershipsTx(ctx context.Context, tx *sql.Tx, deliveryID, alertID, ownerIncidentID string, occurredAt, now time.Time) error {
	rows, err := tx.QueryContext(ctx, `
		...
		WHERE ia.alert_id = ? AND i.id <> ?
		  AND i.status NOT IN ('resolved', 'failed')
		  AND (s.id IS NULL OR s.lifecycle NOT IN ('recovered', 'closed_unknown'))
		ORDER BY i.created_at ASC, i.id ASC`, alertID, ownerIncidentID)
	if err != nil {
		return fmt.Errorf("store: list related recovery memberships: %w", err)
	}
	defer func() { _ = rows.Close() }()
	type membership struct{ id, groupKey, status string }
	var memberships []membership
	for rows.Next() {
		var m membership
		if err := rows.Scan(&m.id, &m.groupKey, &m.status); err != nil {
			return fmt.Errorf("store: scan related recovery membership: %w", err)
		}
		memberships = append(memberships, m)
	}
	if err := rows.Err(); err != nil {
		return fmt.Errorf("store: iterate related recovery memberships: %w", err)
	}
	if err := rows.Close(); err != nil {
		return fmt.Errorf("store: close related recovery memberships: %w", err)
	}

	for _, m := range memberships {
		kind, err := reconcileResolvedMembershipTx(ctx, tx, m.id, m.status, now)
		if err != nil {
			return err
		}
		inputID := "delivery:" + deliveryID + ":membership:" + m.id
		if _, err := tx.ExecContext(ctx, `
			INSERT OR IGNORE INTO situation_input_outbox
				(id, idempotency_key, incident_id, delivery_id, kind, group_key, occurred_at, status)
			VALUES (?, ?, ?, NULL, ?, ?, ?, 'pending')`,
			inputID, inputID, m.id, kind, m.groupKey, canonicalTime(occurredAt)); err != nil {
			return fmt.Errorf("store: insert related recovery situation input: %w", err)
		}
	}
	return nil
}

func reconcileResolvedMembershipTx(ctx context.Context, tx *sql.Tx, incidentID, status string, now time.Time) (string, error) {
	if status != "ready" && status != "analyzed" {
		return "membership_changed", nil
	}
	unresolved, err := unresolvedIncidentMembersTx(ctx, tx, incidentID)
	if err != nil {
		return "", err
	...
	if err != nil {
		return "", fmt.Errorf("store: resolve related incident membership: %w", err)
	}
	changed, err := res.RowsAffected()
	if err != nil {
		return "", fmt.Errorf("store: count resolved related incident membership: %w", err)
	}
	if changed != 1 {
		return "membership_changed", nil
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM incident_triage WHERE incident_id=? AND phase!='exhausted'`, incidentID); err != nil {
		return "", fmt.Errorf("store: clear related incident triage: %w", err)
	}
...
func (s *Store) ApplyCorrelatedDelivery(ctx context.Context, m CorrelatedDeliveryMutation) (CorrelatedDeliveryResult, error) {
	...
	if deliveryStatus == "resolved" {
		if err := reconcileResolvedMembershipsTx(ctx, tx, m.DeliveryID, alertID, m.Incident.ID, deliveryReceivedAt, now); err != nil {
			return CorrelatedDeliveryResult{}, err
		}
	}
