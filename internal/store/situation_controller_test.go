...
func TestLoadSituationDeliveriesKeepsTerminalEpisodeFrozenAfterLinkedRecurrence(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	now := time.Date(2026, 9, 22, 15, 0, 0, 0, time.UTC)
	original := deliveryFixture("delivery-terminal-original", "fp-terminal-shared", now)
	accepted, err := st.AcceptDeliveries(ctx, []DeliveryInput{original})
	if err != nil {
		t.Fatal(err)
	}
	alertID := accepted[0].Alert.ID
	insertIncidentAndDeliveryInput(t, st, "incident-terminal-original", "input-terminal-original", "service=terminal", original.ID, now)
	if _, err := st.DB().ExecContext(ctx, `INSERT INTO incident_alerts (incident_id,alert_id,created_at) VALUES (?,?,?)`,
		"incident-terminal-original", alertID, canonicalTime(now)); err != nil {
		t.Fatal(err)
	}
	if err := st.ApplySituationInput(ctx, claimOneInput(t, st, "terminal-worker", now)); err != nil {
		t.Fatal(err)
	}
	var situationID string
	if err := st.DB().QueryRowContext(ctx, `SELECT situation_id FROM situation_incidents WHERE incident_id='incident-terminal-original'`).Scan(&situationID); err != nil {
		t.Fatal(err)
	}
	terminalAt := now.Add(5 * time.Minute)
	recoveryObservedAt := now.Add(4 * time.Minute)
	if _, err := st.DB().ExecContext(ctx, `UPDATE situations SET lifecycle='recovery_pending', recovery_observed_at=?, grace_until=?, updated_at=? WHERE id=?`,
		canonicalTime(recoveryObservedAt), canonicalTime(terminalAt), canonicalTime(recoveryObservedAt), situationID); err != nil {
		t.Fatal(err)
	}
	if _, err := st.DB().ExecContext(ctx, `
		UPDATE situations SET lifecycle='recovered', terminal_at=?, terminal_reason=NULL, updated_at=? WHERE id=?`,
		canonicalTime(terminalAt), canonicalTime(terminalAt), situationID); err != nil {
		t.Fatal(err)
	}

	// Keep recurrence within the terminal timestamp's second. RFC3339Nano uses
	// variable-width fractions, so terminal filtering must compare chronology
	// rather than raw text.
	recurrenceAt := terminalAt.Add(500 * time.Millisecond)
	recurrence := deliveryFixture("delivery-linked-recurrence", "fp-terminal-shared", recurrenceAt)
	if _, err := st.AcceptDeliveries(ctx, []DeliveryInput{recurrence}); err != nil {
		t.Fatal(err)
	}
	insertIncidentAndDeliveryInput(t, st, "incident-linked-recurrence", "input-linked-recurrence", "service=terminal", recurrence.ID, recurrenceAt)
	if _, err := st.DB().ExecContext(ctx, `INSERT INTO incident_alerts (incident_id,alert_id,created_at) VALUES (?,?,?)`,
		"incident-linked-recurrence", alertID, canonicalTime(recurrenceAt)); err != nil {
		t.Fatal(err)
	}
	...
	if len(deliveries) != 1 || deliveries[0].ID != original.ID {
		t.Fatalf("terminal deliveries = %+v, want only %s", deliveries, original.ID)
	}
}
