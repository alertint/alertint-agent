...
func TestSemanticProfilesUpgradeMigration21Database(t *testing.T) {
	...
	if got != 33 {
		t.Fatalf("MaxSchemaVersion = %d, want 33", got)
	}
	var version int
	if err := st.db.QueryRowContext(ctx, `SELECT MAX(version) FROM schema_migrations`).Scan(&version); err != nil || version != 33 {
		t.Fatalf("applied schema version = %d (err=%v), want 33", version, err)
	}
