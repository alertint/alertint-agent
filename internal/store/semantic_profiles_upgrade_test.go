...
func TestSemanticProfilesUpgradeMigration21Database(t *testing.T) {
	...
	if got != 32 {
		t.Fatalf("MaxSchemaVersion = %d, want 32", got)
	}
	var version int
	if err := st.db.QueryRowContext(ctx, `SELECT MAX(version) FROM schema_migrations`).Scan(&version); err != nil || version != 32 {
		t.Fatalf("applied schema version = %d (err=%v), want 32", version, err)
	}
