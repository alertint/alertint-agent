...
func TestSemanticProfilesUpgradeMigration21Database(t *testing.T) {
	...
	if got != 24 {
		t.Fatalf("MaxSchemaVersion = %d, want 24", got)
	}
	var version int
	if err := st.db.QueryRowContext(ctx, `SELECT MAX(version) FROM schema_migrations`).Scan(&version); err != nil || version != 24 {
		t.Fatalf("applied schema version = %d (err=%v), want 24", version, err)
	}
