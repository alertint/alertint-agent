...
CREATE TABLE situations (
    ...
    CHECK ((lifecycle='active' AND recovery_observed_at IS NULL AND terminal_at IS NULL AND terminal_reason IS NULL) OR
           (lifecycle='recovery_pending' AND recovery_observed_at IS NOT NULL AND terminal_at IS NULL AND terminal_reason IS NULL) OR
           (lifecycle='recovered' AND recovery_observed_at IS NOT NULL AND terminal_at IS NOT NULL AND terminal_reason IS NULL) OR
           (lifecycle='closed_unknown' AND terminal_at IS NOT NULL AND terminal_reason IS NOT NULL))
) STRICT;
CREATE UNIQUE INDEX situations_one_nonterminal_group_idx ON situations(group_key)
    WHERE lifecycle IN ('active','recovery_pending');
CREATE UNIQUE INDEX situations_public_handle_nocase_idx ON situations(public_handle COLLATE NOCASE)
    WHERE public_handle IS NOT NULL;
CREATE INDEX situations_due_idx ON situations(next_assessment_at, retry_at)
    WHERE lifecycle IN ('active','recovery_pending');

CREATE TRIGGER situations_legal_lifecycle_update BEFORE UPDATE OF lifecycle ON situations
WHEN NOT ((OLD.lifecycle='active' AND NEW.lifecycle IN ('active','recovery_pending','closed_unknown')) OR
          (OLD.lifecycle='recovery_pending' AND NEW.lifecycle IN ('recovery_pending','active','recovered','closed_unknown')))
BEGIN SELECT RAISE(ABORT, 'invalid situation lifecycle transition'); END;
CREATE TRIGGER situations_terminal_immutable BEFORE UPDATE ON situations
WHEN OLD.terminal_at IS NOT NULL AND
     (NEW.lifecycle IS NOT OLD.lifecycle OR NEW.terminal_at IS NOT OLD.terminal_at OR NEW.terminal_reason IS NOT OLD.terminal_reason)
BEGIN SELECT RAISE(ABORT, 'terminal situation is immutable'); END;
CREATE TRIGGER situations_public_handle_immutable BEFORE UPDATE OF public_handle ON situations
WHEN OLD.public_handle IS NOT NULL AND NEW.public_handle IS NOT OLD.public_handle
BEGIN SELECT RAISE(ABORT, 'situation public handle is immutable'); END;
...
CREATE INDEX situation_incidents_situation_idx ON situation_incidents(situation_id, attached_at);
CREATE TRIGGER situation_incidents_no_update BEFORE UPDATE ON situation_incidents
BEGIN SELECT RAISE(ABORT, 'situation membership is immutable'); END;
CREATE TRIGGER situation_incidents_no_delete BEFORE DELETE ON situation_incidents
BEGIN SELECT RAISE(ABORT, 'situation membership is immutable'); END;
...
CREATE TABLE situation_input_outbox (
    ...
    kind                 TEXT NOT NULL CHECK (kind IN ('incident_created','membership_changed','incident_ready','finding_persisted','triage_skipped','triage_retry_changed','triage_exhausted','incident_resolved')),
    group_key            TEXT NOT NULL CHECK (group_key <> ''),
