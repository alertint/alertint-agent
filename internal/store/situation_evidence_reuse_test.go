...
func evidenceReuseFixture(t *testing.T) (*Store, situation.Claim, time.Time) {
	...
	claims, err := st.ClaimDueSituations(context.Background(), "reuse-test", now, time.Hour, 1)
	if err != nil || len(claims) != 1 || claims[0].ID != id {
		t.Fatalf("claim evidence fixture: %v, %d claims", err, len(claims))
	}
	claim := situation.Claim{Situation: claims[0], ClaimOwner: *claims[0].LeaseOwner, ClaimToken: claims[0].ClaimToken}
	return st, claim, now
}
...
func TestRetainedEvidenceLoadsPlanMetadata(t *testing.T) {
	st, claim, now := evidenceReuseFixture(t)
	seedEvidenceCycle(t, st, claim, now, "config-a", observationmodel.CapabilityPrometheusQuery, "metric_summary", `{"count":0}`, observationmodel.ResultConfirmedEmpty)
	seedEvidenceCycle(t, st, claim, now.Add(time.Minute), "config-a", observationmodel.CapabilityLokiQuery, "log_summary", `{"count":0}`, observationmodel.ResultConfirmedEmpty)
	in, err := st.LoadReconciliationInput(context.Background(), claim, now.Add(time.Minute))
	if err != nil {
		t.Fatal(err)
	}
	if len(in.Prepared.Runs) != 2 {
		t.Fatalf("retained runs=%d want 2", len(in.Prepared.Runs))
	}
	for _, run := range in.Prepared.Runs {
		p, ok := in.Prepared.PlansByID[run.PlanID]
		if !ok || p.Capability != run.Capability || p.Scope.SubjectID == "" {
			t.Fatalf("missing retained plan metadata: run=%+v plan=%+v", run, p)
		}
	}
}

func TestRetainedDecisionEvidenceReceivesPermanentReferences(t *testing.T) {
	st, claim, now := evidenceReuseFixture(t)
	seedEvidenceCycle(t, st, claim, now, "config-a", observationmodel.CapabilityPrometheusQuery, "metric_summary", `{"count":0}`, observationmodel.ResultConfirmedEmpty)
	at := now.Add(time.Minute)
	seedEvidenceCycle(t, st, claim, at, "config-a", observationmodel.CapabilityLokiQuery, "log_summary", `{"count":0}`, observationmodel.ResultConfirmedEmpty)
	in, err := st.LoadReconciliationInput(context.Background(), claim, at)
	if err != nil {
		t.Fatal(err)
	}
	...
	defer func() { _ = tx.Rollback() }()
	if err := insertPermanentObservationReferencesTx(context.Background(), tx, in.Prepared.CycleID, ObservationReferenceAssessmentAttempt, "attempt-retained", at); err != nil {
		t.Fatal(err)
	}
	var count int
