...
ticking by, and an ordinary recurrence attaching to an already-active
Situation create **nothing** — no entry, no edit, no interruption. A re-fire
during recovery grace is material: it records one recovery-refire transition
and resumes the same Situation. A firing after terminal closure opens a new
root linked to the prior Situation; the terminal root never reopens.
