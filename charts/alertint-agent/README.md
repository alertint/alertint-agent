...
![Version: 0.2.1](https://img.shields.io/badge/Version-0.2.1-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.13.8](https://img.shields.io/badge/AppVersion-0.13.8-informational?style=flat-square)
...
Listed on [Artifact Hub](https://artifacthub.io/packages/helm/alertint-agent/alertint-agent) as an Official repository, published by AlertINT as a verified publisher.
...
| secret | object | `{"create":false,"data":{},"enabled":true,"existingSecret":""}` | Secret providing every *_env value referenced anywhere in `config` below (webhook_token_env, api_key_env, bot_token_env, token_env, ...). All of its keys are exposed to the container via envFrom, so any name you reference in `config` just needs to exist as a key here.  Exactly one of these three shapes is required — the chart fails the render with a clear message otherwise: both create and existingSecret set; neither set while enabled stays true; or enabled: false alongside create/ existingSecret (a contradiction — with enabled false the Deployment never references the built-in Secret at all, so whichever one you configured here would be created/referenced but silently unused).   1) secret.existingSecret: the name of a Secret you manage yourself      (External Secrets, Sealed Secrets, SOPS, etc.) — the recommended path      for anything beyond local testing.   2) secret.create: true with secret.data — the chart creates a plain      Secret from values. Convenient for a quick start; do not commit real      tokens into a values file under version control.   3) secret.enabled: false — a deliberate opt-out of the built-in Secret      entirely (e.g. every *_env value is supplied via extraEnv/extraEnvFrom      instead). No envFrom is rendered in this case. Requires both create      and existingSecret to be left unset. |
| secret.enabled | bool | `true` | Set to false to opt out of the built-in Secret entirely (see mode 3 above). Requires both create and existingSecret to be left unset — combining enabled: false with either of them fails the render. |
