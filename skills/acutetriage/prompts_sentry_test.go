...
func TestRenderSentry_PopulatedSection(t *testing.T) {
	...
	out := UserPrompt(basePack(), "{}", nil, nil, nil, e, nil, nil, VerificationParams{})
...
func TestRenderSentry_NegativeAndDegradedNotes(t *testing.T) {
	...
	out := UserPrompt(basePack(), "{}", nil, nil, nil, zero, nil, nil, VerificationParams{})
	...
	out = UserPrompt(basePack(), "{}", nil, nil, nil, degraded, nil, nil, VerificationParams{})
	if !strings.Contains(out, "Sentry query unavailable (rate-limited)") {
