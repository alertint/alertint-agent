...
// TestFetchChanges_DemoChangesExcludedFromRealIncidents pins the drill
// segregation (ADR-0013/0014): a change event carrying the demo marker never
// enriches a real incident (it would be fictional live evidence lifting the
// confidence cap), but still enriches a Drill.
func TestFetchChanges_DemoChangesExcludedFromRealIncidents(t *testing.T) {
	ctx := context.Background()
	st, _ := store.Open(ctx, ":memory:")
	defer func() { _ = st.Close() }()
	...
	last := first.Add(5 * time.Minute)
	demoTS := first.Add(-5 * time.Minute)
	realTS := first.Add(-10 * time.Minute)
	_ = st.InsertChange(ctx, store.Change{ID: "demo-deploy", Source: "alertint-demo", Kind: "deploy", Title: "deploy checkout v2.3.1",
		Labels: map[string]string{"service": "demo-checkout", store.DemoMarkerLabel: store.DemoMarkerValue}, OccurredAt: demoTS, ReceivedAt: demoTS})
	...
	t.Run("drill incident still sees the planted deploy", func(t *testing.T) {
		drillAlerts := alertsWithLabels(map[string]string{"service": "demo-checkout", store.DemoMarkerLabel: store.DemoMarkerValue})
	...
	t.Run("only demo changes in window notes empty for real incident", func(t *testing.T) {
		firstNarrow := first.Add(-6 * time.Minute) // window catches only the demo deploy
