...
func TestUserPrompt_WithLiveLogsNoCalibration(t *testing.T) {
	out := UserPrompt(basePack(), "{}", nil, liveLogs(), nil, nil, nil, nil, VerificationParams{})
	if strings.Contains(out, "ANNOTATIONS ONLY") {
...
func TestUserPrompt_WithMetricsNoCalibration(t *testing.T) {
	...
	out := UserPrompt(basePack(), "{}", metrics, nil, nil, nil, nil, nil, VerificationParams{})
	if strings.Contains(out, "ANNOTATIONS ONLY") {
...
func TestUserPrompt_DegradedMetricsExemptFromCap(t *testing.T) {
	...
	out := UserPrompt(basePack(), "{}", m, nil, nil, nil, nil, nil, VerificationParams{})
	if strings.Contains(out, "ANNOTATIONS ONLY") {
...
func TestAnnotationsOnlyBasis_DegradedExemptOthersNot(t *testing.T) {
	if annotationsOnlyBasis(&MetricEnrichment{Outcome: OutcomeDegraded}, nil, nil, nil, nil, nil) {
		t.Error("degraded (slow) metrics must be exempt from the annotations-only cap")
	}
	if !annotationsOnlyBasis(&MetricEnrichment{Outcome: OutcomeFailed}, nil, nil, nil, nil, nil) {
		t.Error("a genuine metric failure is annotations-only")
	}
	if !annotationsOnlyBasis(&MetricEnrichment{Outcome: OutcomeEmpty}, nil, nil, nil, nil, nil) {
		t.Error("a queried-empty metric result is annotations-only")
	}
	if !annotationsOnlyBasis(nil, nil, nil, nil, nil, nil) {
		t.Error("no evidence at all is annotations-only")
	}
	if annotationsOnlyBasis(&MetricEnrichment{Snapshots: make([]MetricSnapshot, 1)}, nil, nil, nil, nil, nil) {
...
func TestHasLiveEvidence_MetricsPresenceLiftsCap(t *testing.T) {
	...
	if !hasLiveEvidence(&MetricEnrichment{Snapshots: make([]MetricSnapshot, 1)}, nil, nil, nil, nil) {
		t.Error("one snapshot must count as live evidence")
	}
	if hasLiveEvidence(&MetricEnrichment{Outcome: OutcomeEmpty}, nil, nil, nil, nil) {
